...
def tracking(screen, x, y, yaw, velocity, max_acceleration, dt):
    ...
    ## 좀 더 먼 곳을 목표지점으로 하기 위해서 '+ 15' 로 설정했습니다
    ## 따라서 주차시에도 일찍 회전하게돼서 알맞게 들어갑니다
    target_index = nearest_index + 25 if nearest_index + 25 < len(rx) else len(rx)-1
    ...
    #rospy.loginfo(f"Computed PID: {PID_controller.compute(angle_error, dt)}")
    
    ## -min 에서 min 으로 변경했습니다. angle_error 구하는 곳이 바뀌어서 그렇습니다
    steering_angle = min(max(compute_PID, -50), 50)  # 최대 조향각을 50도로 제한
    #rospy.loginfo(f"Computed PID: {PID_controller.compute(angle_error, dt)}, After Limit: {steering_angle}")
    print("target_index : ", target_index)
    rospy.loginfo(f"yaw: {yaw} //path_angle: {path_angle} //angle error: {angle_error} //After Limit: {steering_angle}")
    # print("x:",x, "  y:",y, "/    target_index :", target_index, "   target_x: ", target_x, "   target_y", target_y)
    ...
    if cal_distance(x, y, P_END[0], P_END[1]) < 65:
        ...
        if reverse_mode:
            speed = velocity - max_acceleration * dt
        else:
            speed = velocity + max_acceleration * dt
    # max(min_speed, max_speed * (1 - abs(steering_angle) / 50))  # 각도에 반비례하는 속도 조정, 최소 속도 적용
    # rospy.loginfo(f"현재 위치: ({x}, {y}) 목표 위치: ({target_x}, {target_y}) 조향 각도: {steering_angle}, 속도: {speed}")
